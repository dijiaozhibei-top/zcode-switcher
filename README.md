...
ZCode Switcher 用于本地保存和管理多个 ZCode 账号，支持额度显示、无感切换账号无需重启，以及 GLM-5.3-Flash 低额度自动切换。
...
| 额度显示 | 显示额度、订阅状态和刷新结果 |
| GLM-5.3-Flash 自动切换 | 额度低于阈值时，自动切到剩余额度更高的账号 |
| 胶囊悬浮窗 | 显示 GLM-5.3-Flash 账号池统计，并支持调整大小 |
...
6. 如果使用 GLM-5.3-Flash，可开启低额度自动切换。当当前账号额度低于阈值时，软件会自动切到剩余额度更高的账号。
